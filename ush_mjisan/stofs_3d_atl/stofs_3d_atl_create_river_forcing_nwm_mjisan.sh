...
# cp files to work dir
 ...
 # Modify how dates are handled to ensure we're using available data
 PDYHH_FCAST_BEGIN=$(date --date="yesterday" +%Y%m%d%H)  # Use yesterday's date
 PDYHH_NCAST_BEGIN=$(date --date="2 days ago" +%Y%m%d%H)
...
# ------> nowcast/forecast cycle(s) & hr
   ...
   COMOUT_PREV="/lfs/h1/nos/ptmp/mansur.jisan/com/stofs/v2.1/stofs_3d_atl.${PDYm1}"
...
# ---------------------------> backup: create list of nwm files
 ...
 if [ ! -d "${COMINnwm}/nwm.${yyyymmdd_today}" ]; then
     echo "WARNING: Today's NWM directory not found, using previous day"
     yyyymmdd_today=${yyyymmdd_prev}
 fi
...
# vsink.th & msource.th are static files for v.6.1 grid
  ...
  if [ ! -z "${list_nwm_files}" ]; then
     ...
     echo "Running Python script with date: ${str_yyyy_mm_dd_hr}"
     echo "Input files available: $(ls nwm_*.conus.nc | wc -l)"
     echo "System memory status before Python:"
     free -h
     vmstat 1 1
     module load python/3.12.0
     
     export PYTHONPATH="${PYstofs3d}:${PYTHONPATH}"
     set -x
     module load python/3.12.0
     ...
     module load python/3.12.0
     #python -v $fn_py_create_river_th $str_yyyy_mm_dd_hr
     timeout 3600 /usr/bin/time -v python $fn_py_create_river_th $str_yyyy_mm_dd_hr
     
     echo "System memory status after Python:"
     free -h
     vmstat 1 1
