...
# ---------------------------> Global Variables
  ...
  cp -f ${fn_input_gen_hotstart} ${dir_wk}/gen_hot_from_nc.in
...
# -------------------------> blend/combine  restart_from_rtofs & hotstart_from_stofs3d
  ...
  if [[ $(find $fn_hotstart_cbn -type f -size  +20G 2>/dev/null) ]]; then
    ...
    msg="Combined rtofs and stofs3d hotstart: ${fn_out_restart_cbn}  was successfully created"
  ...
  if [[ $flag_rst_cbn_success -eq 1 ]]; then
     cp -pfL ${fn_hotstart_cbn} ${fn_restart_rerun} 
  
     msg="fn_restart_rerun of combined rtofs & oper = ${fn_hotstart_cbn} \n"
    ...
    msg="WANRING: Temperature and salinity data were not used to initialize restart file: \n"
    msg+="${fn_restart_rerun}"
